...
| 날짜 | 발표자 | 주차 | 대주제 | 주차별 세부 주제 |
|------|--------|------|--------|------------------|
| - | 재엽 | 1주차 | 자료구조 | 시간복잡도(Big-O), 배열, 연결 리스트, 스택, 큐 |
| - | 형원 | 2주차 | 자료구조 | 해시 테이블(충돌 해결 방법 포함), 트리, 힙 |
| - | 성화 | 3주차 | 자료구조 | 그래프(DFS/BFS 기본 탐색), 각 자료구조별 시간복잡도 총정리표 및 실습 |
| - | 현우 | 4주차 | 네트워크 | OSI 7계층 vs TCP/IP 4계층, TCP와 UDP, 3-way-handshake |
| 1/11 | 용준 | 5주차 | 네트워크 | HTTP/HTTPS, REST API, CDN·Caching·Reverse Proxy (실무 서비스 설계) |
| 1/25 | 재엽 | 6주차 | 운영체제 | 프로세스 vs 스레드, CPU 스케줄링 알고리즘 |
| 2/1 | 재엽 | 7주차 | 운영체제 | 메모리 관리(페이징·세그먼테이션), 스레드 동기화(세마포어·뮤텍스), 교착상태(Deadlock) 발생 조건 및 해결 방법 |
| 2/8 | 형원 | 8주차 | 데이터베이스 | 관계형 데이터베이스, 정규화, 트랜잭션(ACID), 격리 수준 |
| 2/15 | 성화 | 9주차 | 데이터베이스 | 인덱스 원리(B-Tree 등), SQL 최적화, 실행 계획 분석 |
| 2/22 | 현우 | 10주차 | 디자인 패턴 | 디자인 패턴의 개념과 필요성, 싱글톤·팩토리·스트래티지 패턴 |
| 3/1 | 용준 | 11주차 | 디자인 패턴 | 옵저버·프록시 패턴, MVC·MVVM 등 아키텍처 패턴 |
| 3/8 | - | 12주차 | 종합 정리 | 전 주차 핵심 복습 및 CS 기술 면접 대비, 실습 과제 또는 미니 프로젝트 발표 |
...
- **시간:** 매주 일요일 10:00~11:00 (Asia/Tokyo)
- **링크:** https://meet.google.com/zkd-chfm-vyr
...
-   **스터디 시간:** 매주 일요일 10:00 (Asia/Tokyo)
